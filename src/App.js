    ...
    return (
      <div className="App">
        ...
        <div>
          ...
          <div className='test-wrapper'>
            <div className='test-inside2'>
              <h4>About this app</h4>
              <p>In Daniel Kahneman’s magnificent book <i>Thinking Fast and Slow</i>, he dedicates a chapter to the utility of using simple algorithms to replace subjective judgements in all sorts of applications.  He presents research indicating how simple combinations of scores (algorithms) can provide better forecasts than even expert opinions in a wide variety of fields, from wine pricing to medical diagnoses.  The simplest such algorithm that could be applied to fantasy football would probably be the way your league calculates fantasy points - summing up the various categories into a single score.  But fantasy points focuses too much on outcomes and not enough on process - it doesn't take any sort of efficiency or opportunity metrics (yards per carry, targets, etc.) into account, limiting it's value as a predictor of the future.  This app allows users to create their own simple algorithm, including efficiency and opportunity metrics, run it on 2018 statistics, and use the results of that algorithm to find value for 2019 fantasy drafts.  A user will create a custom-weighted combination of statistics, and a player’s algorithm score (AScore) will be graphed on the y-axis, against their 2019 average draft position (ADP) on the x-axis.  Players with a higher/lower AScore than their neighbors will then represent potential value picks/players to avoid in drafts.  Obviously there are many factors that define a player’s performance other than last year’s statistics, but my hope is that this site will allow fantasy managers to at least identify players worthy of further research (it also doubles as a handy data visualizer.  Just include a single stat in your algorithm, don't normalize, and you've created a graph of how each player performed in that category last year).</p>
            </div>
          </div>
        </div>
      </div>
