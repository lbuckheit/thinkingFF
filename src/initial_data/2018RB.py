...
q.player(position='RB' or 'UNK')
RBs = {}

#TODO - REMOVE THIS SPECIAL SAQUON PART, NO LONGER NECESSARY
qq = nfldb.Query(db)
qq.game(season_year=2018, season_type='Regular')
...
#This is a hacky workaround to the fact that Melvin and Zeke aren't on teams right now
zz = nfldb.Query(db)
zz.game(season_year=2018, season_type='Regular')
zz.player(position='UNK')
for pp in zz.sort('rushing_yds').limit(2).as_aggregate():
